from imagen_hub.loader.benchmark_loader import dump_dataset_info, load_custom_text_guided_ig_dataset
...
def infer_custom_text_guided_ig_bench(model,
                    result_folder: str = 'results',
                    experiment_name: str = "Exp_Custom-Text-Guided_IG",
    ...
    """
    dataset, dataset_name = load_custom_text_guided_ig_dataset(with_name_att=True)
    data = dataset['test']
    del dataset
    ...
    if overwrite_inputs or not os.path.exists(os.path.join(result_folder, experiment_name, 'dataset_lookup.json')):
        dump_dataset_info(data,
                          required_attributes=['prompt', 'category'],
